...
highest_accuracy = 0
best_model = build_model(1, 1, 10, 1)
...
for layers in n_hidden:                          #looking or the best parameters w.r.t the number of hidden layers
    for neuron in n_neurons:                      #looking or the best parameters w.r.t the number of neurons
        for rate in l_rate:                  #looking or the best parameters w.r.t the learning rate
            ...
            model = build_model(layers, neuron, 10, rate)
            #-->add your Pyhton code here
            ...
            for i,prediction in enumerate(class_predicted):
                if prediction == y_test[i]:
                    sum += 1
            accuracy = sum/len(class_predicted)

            if accuracy > highest_accuracy:
                print("Highest accuracy so far: " + str(highest_accuracy))
                print(f"Parameters: Number of Hidden Layers: {layers} number of neurons: {neuron} learning rate: {rate}")
                print()
